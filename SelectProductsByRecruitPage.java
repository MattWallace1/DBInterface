 ...
 * @author joe_admin
 */
public class SelectProductsByRecruitPage extends JFrame implements ActionListener{
    ...
    public SelectProductsByRecruitPage(Image iconImage){
        this.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        this.setLayout(new GridBagLayout());
        this.setPreferredSize(new Dimension(500,300));
        _iconImage = iconImage;
        this.setIconImage(_iconImage);
        this.setTitle("See Products Used by a Recruit");
        ...
        lbldesc = new JLabel("Enter a recruit's id OR full name if id is unknown:");
        lblid = new JLabel("id:");
    ...
    public void actionPerformed(ActionEvent e){
        if (e.getSource() == btnRetrieve){
            if (isValidInput()){
                ...
                Connect conn = new Connect();
                String[][] data = conn.selectProductsByRecruit(id, name);
                if (!(data == null)){
                    ...
                    String title = "All Products Used by ";
                    if (id == null || id.equals("")){
                        title += name;
                    }else{
                        title += conn.getRecruitNameFromId(id) + ", ID: " + id;
                    }
                    String[] colnames = {"Product ID", "PI", "Description", "Test Mode", "Site Location", "Study ID", "PSN"};
